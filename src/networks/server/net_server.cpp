...
namespace net
{
    ...
    template <typename T>
    IServer<T>::IServer(uint16_t port) : m_asioAcceptor(m_asioContext, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port))
    {
    }
    ...
    {
        m_asioAcceptor.async_accept([this](asio::error_code & ec, tcp::socket socket))
        {
            ...
            if (!ec)
            {
                ...
                std::share_ptr<session<T>> newConnection =
                    std::make_shared<session<T>>(session<T>::owner::server,
                                                 m_asioContext,
                                                 std::move(socket),
                                                 m_qMessagesIn);
    ...
    template <typename T>
    void IServer<T>::MessageClient(std::shared_ptr<session<T>> client, const message<T> &msg)
    {
    ...
    template <typename T>
    void IServer<T>::MessageAllClients(const message<T> &msg, std::shared_ptr<session<T>> pIgnoreClient = nullptr)
    {
