    ...
    : wxWindow()
{
    ...
    auto ClientButton = new wButton(MainWindowPanel, wxID_ANY, "CLIENT", wxDefaultPosition, wxSize(260, 45));
    ClientButton->Bind(wxEVT_LEFT_DOWN, [](wxMouseEvent &e)
                       {
        wxMessageBox("Client");
        e.Skip(); });
    auto ServerButton = new wButton(MainWindowPanel, wxID_ANY, "SERVER", wxDefaultPosition, wxSize(260, 45));
