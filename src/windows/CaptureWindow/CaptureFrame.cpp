    ...
    : wxFrame(nullptr, wxID_ANY, title, pos, size)
{
    ...
    // Tạo một wxBoxSizer chứa nút "Start Capture" và "Stop Capture"
...
void CaptureFrame::OnCaptureScreen(wxTimerEvent &e)
{
    if (CAPTURING)
    {
        ...
        double scaleX = static_cast<double>(CapturePanel->GetSize().GetWidth()) / screenWidth;
        double scaleY = static_cast<double>(CapturePanel->GetSize().GetHeight()) / screenHeight;
        ...
        memDC.Blit(0, 0, screenWidth, screenHeight, &screenDC, frameRect.x, frameRect.y);
